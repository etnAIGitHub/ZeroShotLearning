...
class BenedettiniDataset(Dataset):
    ...
    def createTargetList(self):
        ...
        for i in range(100, 167):
            try:
                ...
                target_labels = genfromtxt(
                    path.join(self.target_dir, "labels/%d.txt" % i)
                )
    ...
    def createSupportDict(self):
        ...
        for key, value in labels_dict.items():
            support_dict[key] = [
                filename
                for filename in listdir(self.support_dir)
                if filename.startswith(str(value))
            ][0]
    ...
    def getSupportSet(self):
        ...
        for label, support_image_path in self.support_dict.items():
            support_image = self.transform(
                path.join(self.support_dir, support_image_path)
            )
...
if __name__ == "__main__":
    b = BenedettiniDataset(
        r"E:\Sources\Repos\filtered-benedettini\Object Retrieval\Monastero dei Benedettini"
    )
