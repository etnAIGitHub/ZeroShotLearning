...
from Pytorch.utils import Normalize_bbox_to_0_1, Convert_bbox_from_TLWH_to_TLBR, clamp, Bbox_clamp
...
class ZERO_SHOT_ROI_DATASET(Dataset):
    def __init__(self,
        root_dirpath,
        images_dirpath,
        annotations_path,
    ...
    def Get_random_image_and_bbox_from_class(self, class_idx):
        ...
        D_item_albumentation = {'image': np.array(item_PIL_image), 'bboxes': [item_bbox], self.category_id_key: [0]}
        ...
        return D_item_transformed['image'] , T_bbox_normalized
    ...
    def __getitem__(self, index):
        ...
        L_support_set = []
        for support_idx in L_support_idx:
            T_support_image, support_bbox = self.Get_random_image_and_bbox_from_class(support_idx)
            L_support_set.append((T_support_image, support_bbox))

        target_idx = np.random.choice(len(L_support_idx))
        target_class_idx = L_support_idx[target_idx]
        T_target_image, T_target_bbox_normalized = self.Get_random_image_and_bbox_from_class(target_class_idx)

        return (L_support_set, (T_target_image, T_target_bbox_normalized)), target_idx
...
if __name__ == "__main__":
    DSET_training = ZERO_SHOT_ROI_DATASET(
        ...
        L_classes_idx_filter = list(range(12)),
        support_set_size=10,
        image_id_key="path",
